...
private:
    ...
    // Нам нужно будет понимать високосный ли перед нами год, корректную ли дату
    // ввели:
    ...
    // Помимо прочего будет полезно уметь переводить текущую дату в число —
    // количество дней от 01.01.1970 и обратно.
    ...
    void SetFromDays(int inp_days)
    {
        ...
        while (inp_days > DaysPassedToMonth(GetMonth() + 1,
                                            GetYear())) {  // определяем месяц
            ++m;
        }

        d = inp_days -
            DaysPassedToMonth(GetMonth(), GetYear());  // остаток в дни
    }
